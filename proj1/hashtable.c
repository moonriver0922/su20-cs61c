...
void insertData(HashTable *table, void *key, void *data) {
  ...
  hashcode = hashcode % table->size;
  struct HashBucket *bucket = (struct HashBucket *)malloc(sizeof(struct HashBucket));
  bucket->data = data;
  bucket->key = key;
  bucket->next = NULL;
  if (!table->data[hashcode])
  {
	 table->data[hashcode] = bucket;
  }
  else {
	  bucket->next = table->data[hashcode];
	  table->data[hashcode] = bucket;
  }
}
...
void *findData(HashTable *table, void *key) {
  ...
  unsigned int targetHashcode = table->hashFunction(key) % table->size;
  void *data = NULL;
  struct HashBucket *bucket = table->data[targetHashcode];
  while (bucket != NULL)
  {
